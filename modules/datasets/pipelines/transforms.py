...
class RandomCrop(object):
    ...
    Args:
        size (sequence or int, required): Desired output size of the crop. If size is an
        ...
        pad_if_needed (boolean, optional): It will pad the image if smaller than the
            ...
            Default: False.
        pad_val (Number | Sequence[Number], optional): Pixel pad_val value for constant
            fill. If a tuple of length 3, it is used to pad_val R, G, B
            channels respectively. Default: 0.
        padding_mode (str, optional): Type of padding. Defaults to "constant". Should
    ...
    def get_params(img, output_size):
        ...
        Args:
            img (ndarray, required): Image to be cropped.
            output_size (tuple, required): Expected output size of the crop.
        Returns:
            tuple: Params (ymin, xmin, target_height, target_width) to be
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if self.pad_if_needed and img.shape[0] < self.size[0]:
                img = mmcv.impad(img,
                                 padding=(0, self.size[0] - img.shape[0], 0, self.size[0] - img.shape[0]),
            ...
            if self.pad_if_needed and img.shape[1] < self.size[1]:
                img = mmcv.impad(img,
                                 padding=(self.size[1] - img.shape[1], 0, self.size[1] - img.shape[1], 0),
                                 pad_val=self.pad_val,
                                 padding_mode=self.padding_mode)

            ymin, xmin, height, width = self.get_params(img, self.size)
            results[key] = mmcv.imcrop(img, np.array([
...
class RandomResizeCrop(object):
    ...
    Args:
        size (sequence | int, required): Desired output size of the crop. If size is an
            int instead of sequence like (h, w), a square crop (size, size) is made.
        scale (tuple, optional): Range of the random size of the cropped image compared
            to the original image. Defaults to (0.08, 1.0).
        ratio (tuple, optional): Range of the random aspect ratio of the cropped image
            compared to the original image. Defaults to (3. / 4., 4. / 3.).
        max_attempts (int, optional): Maximum number of attempts before falling back to
            Central Crop. Defaults to 10.
        efficientnet_style (bool, optional): Whether to use efficientnet style Random
            ResizedCrop. Defaults to False.
        min_covered (Number, optional): Minimum ratio of the cropped area to the original
             area. Only valid if efficientnet_style is true. Defaults to 0.1.
        crop_padding (int, optional): The crop padding parameter in efficientnet style
            center crop. Only valid if efficientnet_style is true.
            Defaults to 32.
        interpolation (str, optional): Interpolation method, accepted values are
            'nearest', 'bilinear', 'bicubic', 'area', 'lanczos'. Defaults to
            'bilinear'.
        backend (str, optional): The image resize backend type, accepted values are
    ...
    def get_params(img, scale, ratio, max_attempts=10):
        ...
        Args:
            img (ndarray, required): Image to be cropped.
            scale (tuple, required): Range of the random size of the cropped image
                compared to the original image size.
            ratio (tuple, required): Range of the random aspect ratio of the cropped
                image compared to the original image area.
            max_attempts (int, optional): Maximum number of attempts before falling back
    ...
    def get_params_efficientnet_style(img,
        ...
        Args:
            img (ndarray, required): Image to be cropped.
            size (sequence, required): Desired output size of the crop.
            scale (tuple, required): Range of the random size of the cropped image
                compared to the original image size.
            ratio (tuple, required): Range of the random aspect ratio of the cropped
                image compared to the original image area.
            max_attempts (int, optional): Maximum number of attempts before falling back
                to central crop. Defaults to 10.
            min_covered (Number, optional): Minimum ratio of the cropped area to the
                original area. Only valid if efficientnet_style is true.
                Defaults to 0.1.
            crop_padding (int, optional): The crop padding parameter in efficientnet
...
class RandomGrayscale(object):
    ...
    Args:
        gray_prob (float, optional): Probability that image should be converted to
    ...
    def __call__(self, results):
        """
        Args:
            results (ndarray, required): Image to be converted to grayscale.
        Returns:
...
class RandomFlip(object):
    ...
    Args:
        flip_prob (float, optional): The probability of the image being flipped. Default: 0.5
...
class RandomErasing(object):
    ...
    Args:
        erase_prob (float, optional): Probability that image will be randomly erased.
            Default: 0.5
        min_area_ratio (float, optional): Minimum erased area / input image area
            Default: 0.02
        max_area_ratio (float, optional): Maximum erased area / input image area
            Default: 0.4
        aspect_range (sequence | float, optional): Aspect ratio range of erased area.
            if float, it will be converted to (aspect_ratio, 1/aspect_ratio)
            Default: (3/10, 10/3)
        mode (str, optional): Fill method in erased area, can be:
            - const (default): All pixels are assign with the same value.
            - rand: each pixel is assigned with a random value in [0, 255]
        fill_color (sequence | Number, optional): Base color filled in erased area.
    ...
    def __init__(self,
        ...
        assert min_area_ratio <= max_area_ratio, 'min_area_ratio should be smaller than max_area_ratio'
        if isinstance(aspect_range, float):
            aspect_range = min(aspect_range, 1 / aspect_range)
            aspect_range = (aspect_range, 1 / aspect_range)
        assert isinstance(aspect_range, Sequence) and len(aspect_range) == 2 and all(
            isinstance(x, float) for x in aspect_range), 'aspect_range should be a float or Sequence with two float.'
        assert all(x > 0 for x in aspect_range), 'aspect_range should be positive.'
        assert aspect_range[0] <= aspect_range[1], 'In aspect_range (min, max), min should be smaller than max.'
        assert mode in ['const', 'rand']
        if isinstance(fill_color, Number):
            fill_color = [fill_color] * 3
        assert isinstance(fill_color, Sequence) and len(fill_color) == 3 and all(
            isinstance(x, Number) for x in fill_color), 'fill_color should be a float or Sequence with three int.'
        if fill_std is None:
            if isinstance(fill_std, Number):
                fill_std = [fill_std] * 3
            assert isinstance(fill_std, Sequence) and len(fill_std) == 3 and all(
                isinstance(x, Number) for x in fill_std), 'fill_std should be a float or Sequence with three int.'
    ...
    def _fill_pixels(self, img, top, left, h, w):
        if self.mode == 'const':
            patch = np.empty((h, w, 3), dtype=np.uint0)
            patch[:, :] = np.array(self.fill_color, dtype=np.uint0)
    ...
    def __call__(self, results):
        """Call function to erase image in random patch.
        Args:
            results (dict, required): Results dict from pipeline
        Returns:
...
class Pad(object):
    """Pad images.
    Args:
        size (tuple[int] | None, optional): Expected padding size (h, w). Conflicts with
                pad_to_square. Defaults to None.
        pad_to_square (bool, optional): Pad any image to square shape. Defaults to False.
        pad_val (Number | Sequence[Number], optional): Values to be filled in padding
            areas when padding_mode is 'constant'. Default to 0.
        padding_mode (str, optional): Type of padding. Should be: constant, edge,
    ...
    def __init__(self,
        ...
        assert (size is None) ^ (pad_to_square is False), \
            f'Only one of [size, pad_to_square] should be given, ' \
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if self.pad_to_square:
                target_size = tuple(max(img.shape[0], img.shape[1]) for _ in range(2))
            else:
                target_size = self.size
            img = mmcv.impad(img, shape=target_size, pad_val=self.pad_val, padding_mode=self.padding_mode)
            results[key] = img
...
class Resize(object):
    """Resize images.
    Args:
        size (int | tuple, required): Images scales for resizing (h, w).
        ...
        interpolation (str, optional): Interpolation method. For "cv2" backend, accepted
            values are "nearest", "bilinear", "bicubic", "area", "lanczos". For
            "pillow" backend, accepted values are "nearest", "bilinear",
            "bicubic", "box", "lanczos", "hamming". Default to 'bilinear'.
        adaptive_side(str, optional): Adaptive resize policy, accepted values are
            "short", "long", "height", "width". Default to "short".
        backend (str, optional): The image resize backend type, accepted values are
            `cv2` and `pillow`. Default: `cv2`.
    """

    def __init__(self, size, interpolation='bilinear', adaptive_side='short', backend='cv2'):
...
class CenterCrop(object):
    ...
    Args:
        crop_size (int | tuple, required): Expected size after cropping with the format of (h, w).
        efficientnet_style (bool, optional): Whether to use efficientnet style center crop. Defaults to False.
        crop_padding (int, optional): The crop padding parameter in efficientnet style
            center crop. Only valid if efficientnet style is True. Defaults to 32.
        interpolation (str, optional): Interpolation method, accepted values are
...
class Normalize(object):
    """Normalize the image.
    Args:
        mean (sequence, required): Mean values of 3 channels.
        std (sequence, required): Std values of 3 channels.
        to_rgb (bool, optional): Whether to convert the image from BGR to RGB. Default is true.
    """
...
class Lighting(object):
    ...
    Args:
        eigval (list, required): the eigenvalue of the convariance matrix of pixel values, respectively.
        eigvec (list[list], required): the eigenvector of the convariance matrix of pixel values, respectively.
        alphastd (float, optional): The standard deviation for distribution of alpha. Default to 0.1
        to_rgb (bool, optional): Whether to convert img to rgb. Default to True.
    """

    def __init__(self, eigval, eigvec, alphastd=0.1, to_rgb=True):
        assert isinstance(eigval, list), f'eigval must be of type list, got {type(eigval)} instead.'
        assert isinstance(eigvec, list), f'eigvec must be of type list, got {type(eigvec)} instead.'
        for vec in eigvec:
...
class Albu(object):
    ...
    Args:
        transforms (list[dict], required): A list of albu transformations
        keymap (dict, optional): Contains {'input key':'albumentation-style key'}. Default to None
        update_pad_shape (bool, optional): Default to False.
    """
    ...
    def albu_builder(self, cfg):
        ...
        Args:
            cfg (dict, required): Config dict. It should at least contain the key "type".
        Returns:
    ...
    def mapper(d, keymap):
        ...
        Args:
            d (dict, required): old dict
            keymap (dict, required): {'old_key':'new_key'}
        Returns:
            :dict: new dict.
        """
