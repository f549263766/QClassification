...
class ConcatDataset(_ConcatDataset):
    ...
    Args:
        datasets (list[:obj:`BaseDataset`], required): A list of datasets.
        separate_eval (bool, optional): Whether to evaluate the results
    ...
    def get_cat_ids(self, idx):
        ...
        Args:
            idx (int, required): The index of datasets.
        Return:
             :obj: The one of datasets with index.
        """
        if idx < 0:
            if -idx > len(self):
                raise ValueError(
    ...
    def evaluate(self, results, *args, indices=None, logger=None, **kwargs):
        ...
        Args:
            results (list[list | tuple], required): Testing results of the dataset.
...
class RepeatDataset(object):
    ...
    epochs.
    Args:
        dataset (obj:`BaseDataset`, required): The dataset to be repeated.
        times (int, required): Repeat times.
    """
...
@DATASETS.register_module()
class ClassBalanceDatasets(object):
    ...
    Args:
        dataset (obj:`BaseDataset`, required): The dataset to be repeated.
        oversample_thr (float, required): frequency threshold below which data is
    ...
    def _get_repeat_factors(self, dataset, repeat_thr):
        """Compute the repeat factor with dataset.
        Args:
            dataset (obj:`BaseDataset`, required): The dataset to be repeated.
            repeat_thr (float, required): frequency threshold below which data is repeated.
        Return:
            :list: The repeat factor for each category.
        """
        ...
        # 3. For each image I and its labels L(I), compute the image-level repeat factor: r(I) = max_{c in L(I)} r(c)
        repeat_factors = []
        for idx in range(num_images):
            cat_ids = set(self.dataset.get_cat_ids(idx))
            repeat_factor = max(
...
class KFoldDataset:
    ...
    Args:
        dataset (obj:`BaseDataset`, required): The dataset to be divided.
        fold (int, optional): The fold used to do validation. Defaults to 0.
        num_splits (int, optional): The number of all folds. Defaults to 5.
        test_mode (bool, optional): Use the training dataset or validation dataset.
            Defaults to False.
